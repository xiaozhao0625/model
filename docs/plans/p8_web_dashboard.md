...
P8.1 在不修改路由、API client 协议字段、状态枚举、bucket 原始值、worker_type、provider_type、capabilities 和后端逻辑的前提下，将 Web Console 可见文案中文化：

- 左侧导航、顶部标题、页面标题、卡片标题、表格列名、按钮和操作提示。
- Run 状态、bucket、Worker 类型、Worker 能力、provider 类型、操作按钮展示映射。
- Upload & Cleanup 页面中关于 `capture_completed`、`upload_pending`、`uploaded_confirmed`、`local_deleted`、`completed` 的状态说明。
- Model Gateway 页面中的 provider 能力、安全拦截和审计摘要。
- Settings 页面只读配置说明。
