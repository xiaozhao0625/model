...
$SumatraSettings = Join-Path (Split-Path -Parent $SumatraExe) "SumatraPDF-settings.txt"
...
$PumpStarted = $false
$SumatraProcess = $null
$MetadataFile = Join-Path $AppShotHome "cache\frame-pump\sumatrapdf_frame_pump_capture.json"
$BeforePowerPolicy = Join-Path $AppShotHome "logs\power_policy_before_capture.json"

function Set-SumatraCaptureSetting {
  param(
    [string]$Path,
    [string]$Key,
    [string]$Value
  ...
  if (-not (Test-Path -LiteralPath $Path)) {
    Set-Content -LiteralPath $Path -Value @("# App Shot SumatraPDF capture settings") -Encoding UTF8
  }
...
try {
  ...
  & powershell -NoProfile -ExecutionPolicy Bypass -File $DiagnoseInput -TargetProcessName "SumatraPDF" | Out-Null
  ...
  Set-SumatraCaptureSetting -Path $SumatraSettings -Key "UiLanguage" -Value "en"
  Set-SumatraCaptureSetting -Path $SumatraSettings -Key "ShowMenubar" -Value "true"
  Set-SumatraCaptureSetting -Path $SumatraSettings -Key "ShowToolbar" -Value "true"
  # Contract strings for tests: UiLanguage = en / ShowMenubar = true
...
pattern = re.compile(r"sumatra|pdf", re.I)
...
while time.time() < deadline and not found:
    ...
    time.sleep(0.5)
if not found:
    raise SystemExit("sumatrapdf_window_not_found")
hwnd = found[0]
user32.SetWindowPos(hwnd, 0, 0, 0, 1200, 900, SWP_SHOWWINDOW)
user32.SetForegroundWindow(hwnd)
print(hwnd)
'@
  $PositionScriptPath = Join-Path $AppShotHome "logs\sumatrapdf_position_window.py"
  ...
  if ($LASTEXITCODE -ne 0) {
    throw "Failed to position SumatraPDF window"
  }

  & powershell -NoProfile -ExecutionPolicy Bypass -File $StartPump -WindowTitlePattern "Sumatra|sumatra|PDF" -MetadataFile $MetadataFile -IntervalSeconds 0.5 | Out-Null
  $PumpStarted = $true
  Start-Sleep -Seconds 2
...
metadata_file = Path(os.environ["SUMATRA_METADATA_FILE"])
start_pump_script = Path(os.environ["SUMATRA_START_PUMP_SCRIPT"])
stop_pump_script = Path(os.environ["SUMATRA_STOP_PUMP_SCRIPT"])
heartbeat_file = Path(os.environ["SUMATRA_HEARTBEAT_FILE"])
max_seconds = int(os.environ["SUMATRA_MAX_SECONDS"])
min_processed = int(os.environ["SUMATRA_MIN_PROCESSED"])
target_accepted_min = int(os.environ["SUMATRA_TARGET_ACCEPTED_MIN"])
target_accepted_max = int(os.environ["SUMATRA_TARGET_ACCEPTED_MAX"])
max_actions = int(os.environ["SUMATRA_MAX_ACTIONS"])
...
run = runtime.create_run(
    V3TaskConfig(
        app_name="sumatrapdf_real_auto_explore_sample",
        app_type="pc_app",
        target_language="en",
...
def restart_frame_pump_if_stale():
    ...
    os.system(
        ...
        f'-WindowTitlePattern "Sumatra|sumatra|PDF" -MetadataFile "{metadata_file}" -IntervalSeconds 0.5 >NUL'
    )
...
def ui_state_for_label(label):
    ...
    if "setting" in lowered or "preference" in lowered:
...
'@
  $DriverPath = Join-Path $AppShotHome "logs\sumatrapdf_real_capture_driver.py"
  [System.IO.File]::WriteAllText($DriverPath, $driver, [System.Text.Encoding]::UTF8)
  $env:SUMATRA_METADATA_FILE = $MetadataFile
  $env:SUMATRA_START_PUMP_SCRIPT = $StartPump
  $env:SUMATRA_STOP_PUMP_SCRIPT = $StopPump
  $env:SUMATRA_HEARTBEAT_FILE = Join-Path $AppShotHome "logs\frame_pump_heartbeat.json"
  $env:SUMATRA_MAX_SECONDS = [string]$MaxSeconds
  $env:SUMATRA_MIN_PROCESSED = [string]$MinProcessed
  $env:SUMATRA_TARGET_ACCEPTED_MIN = [string]$TargetAcceptedMin
  $env:SUMATRA_TARGET_ACCEPTED_MAX = [string]$TargetAcceptedMax
  $env:SUMATRA_MAX_ACTIONS = [string]$MaxActions
  Set-Location $ProjectRoot
  & $Python $DriverPath
  if ($LASTEXITCODE -ne 0) {
    throw "SumatraPDF capture driver failed"
  }
} finally {
  if ($PumpStarted) {
