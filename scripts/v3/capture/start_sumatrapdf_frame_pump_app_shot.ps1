param(
  ...
  [string]$OutputDir = "",
  [string]$WindowTitlePattern = "Sumatra|sumatra|PDF",
  [string]$MetadataFile = "",
...
$PidFile = Join-Path $PumpRoot "sumatrapdf_frame_pump.pid"
$LogFile = Join-Path $PumpRoot "sumatrapdf_frame_pump.log"
$RunnerPath = Join-Path $PumpRoot "sumatrapdf_frame_pump.py"
$HeartbeatFile = Join-Path $AppShotHome "logs\frame_pump_heartbeat.json"
if ([string]::IsNullOrWhiteSpace($MetadataFile)) {
  $MetadataFile = Join-Path $PumpRoot "sumatrapdf_frame_pump_capture.json"
}
...
if (Test-Path -LiteralPath $PidFile) {
  ...
  if ($existingPid -and (Get-Process -Id ([int]$existingPid) -ErrorAction SilentlyContinue)) {
    throw "SumatraPDF frame pump is already running: pid $existingPid"
  }
...
def main() -> int:
    ...
    try:
        while True:
            ...
            frame_path = output_dir / f"sumatra_frame_{stamp}_{frame_index:06d}.png"
            try:
...
if (!(Get-Process -Id $process.Id -ErrorAction SilentlyContinue)) {
  throw "SumatraPDF frame pump failed to start. See $LogFile"
}
