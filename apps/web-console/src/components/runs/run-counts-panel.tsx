  ...
  return (
    <div>
      <div className="mb-3 flex items-end justify-between">
        <div>
          <p className="text-sm text-slate-400">valid_total 达标进度</p>
          <p className="mt-1 text-2xl font-semibold text-slate-50">{formatNumber(run.valid_total)}</p>
        </div>
        <p className="font-mono text-sm text-slate-500">目标进度 {progress}%</p>
      </div>
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
        {[
          ...
          <div key={label} className="rounded-lg border border-slate-800 bg-slate-950 p-3">
            <p className="text-slate-400">{bucketLabels[String(label)] || label}</p>
