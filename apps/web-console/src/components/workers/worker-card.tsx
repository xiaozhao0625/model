    ...
    <article className="rounded-[10px] border border-slate-800 bg-slate-900 p-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">{worker.machine_name || "unassigned machine"}</p>
        </div>
      ...
      <dl className="mt-4 grid grid-cols-2 gap-3 text-sm">
        <div>
          ...
          <dd className="mt-1 text-slate-200">{worker.type}</dd>
        </div>
        <div>
          <dt className="text-slate-500">Current</dt>
          <dd className="mt-1 font-mono text-slate-200">{worker.current_run_id || "idle"}</dd>
        </div>
