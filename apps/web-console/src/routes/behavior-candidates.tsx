...
export function BehaviorCandidatesRoute() {
  ...
  return (
    <div>
      <PageHeader title="行为候选" description="人工审核 P12 行为包候选。pending_review 才允许 approve/reject；rollback 需要确认。" />
      <Card title="候选列表" eyebrow="behavior_learning candidates">
        <DataTable columns={["候选包 ID", "基础包", "类型", "版本", "状态", "来源 run", "回滚目标", "操作"]}>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        {candidates.map((candidate) => (
          <Card key={`${candidate.candidate_pack_id}-detail`} title={candidate.candidate_pack_id} eyebrow="问题 / 建议">
            <p className="text-sm text-slate-500">rejected 不可启用；approved 只显示 enabled，不会自动进入生产执行。</p>
